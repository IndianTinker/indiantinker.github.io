...
Here obviously we will do things manually. But instead of  using twezzers we will use a hacked Aquarium Pump. Does not seem much of an upgrade?

Well, it certainly is. The amount of dextrity required is fairly reduced and so is the frustation of tapping SMD components to have their *sunny side up* :P
...
The catch is with the needle. The usual needle a chemist has is made for epidermal use and hence has a chiseled sort of tip, which is useless for picking components. I wanted to get an "applicator" needle with a blunt tip usually stocked by ink fillers of inkjet printers. Nobody gave me one. Dremel prevailed and i sanded the needle to flatness. Then bent it a bit for an easier angle.
